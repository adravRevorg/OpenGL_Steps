...
void createShaders() {
	...
	lightShader = new Shader("vertex.glsl", "fragment1.glsl");
}
...
- Basic Light Scene
- Step 1:
  Need an object to cast light on - using the cube
  A Shader for the objects

- Step 2:
  Need a light source - for now using a cube again
  A new VAO for the light source. VBO is same, so just reuse
  A shader for the light objects

- Vertex shader can be same for both, but have different fragment shaders
- Display both using respective shaders
- be careful with assigning VAOs, etc and using the correct shader
